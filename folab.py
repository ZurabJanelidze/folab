...
A small experimental proof checker for a bracket-language presentation of
first-order logic.

The module lets a user declare variables, operations, and relations, then build
Fitch-style proofs with commands such as:

    X = Proof(L)
    X.assume("[A] and [B]")
    X.conclude("A")

or with subproof goals:

    Y = Proof(L, goal="[A] implies [A]", parent=X)
    Y.assume("A")
    Y.conclude("A")
    X.goal(Y)

As proof lines are added, the nested proof is printed in the console. If a line
is invalid, a ProofError is raised and the line is not added.

This is intentionally conservative: it checks a useful core of the rules, but it
is not meant to be a complete industrial-strength theorem prover. Rule-specific
arguments are supported for rules where the intended source line is otherwise
ambiguous, such as universal elimination and equality elimination.
"""
...
def matching_pairs(s: str) -> dict[int, int]:
    ...
    for i, ch in enumerate(s):
        if ch == "[":
            ...
            if not stack:
                raise SyntaxErrorFOL(f"Unmatched closing bracket at position {i} in {s!r}.")
            j = stack.pop()
            pairs[j] = i
    if stack:
        raise SyntaxErrorFOL(f"Unmatched opening bracket at position {stack[-1]} in {s!r}.")
    return pairs
...
def is_atomic_expression(s: str) -> bool:
    """Non-empty expression whose brackets, if any, are all placeholders []."""
    s = norm(s)
    if not s:
        return False
    try:
...
def fill_template(template: str, children: Iterable[str]) -> str:
    ...
    if result.count("[]") != len(children):
        raise SyntaxErrorFOL("Wrong number of children for template.")
...
class FirstOrderLanguage:
    ...
    variables: bracketless atomic expressions, e.g. ['x', 'y', 'z'].
    operations: atomic templates for operations, e.g. ['0', '[]+[]', 'S[]'].
    relations: atomic templates for relations other than equality may be added;
               equality '[]=[]' is included automatically unless already given.

    Logical constructors are fixed:
        true, false, [] and [], [] or [], [] implies [],
        for every [] we have [], there exists [] such that [].
    """
    ...
    def _validate_language(self) -> None:
        classes = [self.variables, self.operations, self.relations, self.logical_constructors]
        names: list[str] = []
        for cls in classes:
            for item in cls:
        ...
        if len(set(names)) != len(names):
            ...
            raise SyntaxErrorFOL(f"Language classes are not disjoint: {duplicates}")
    ...
    def substitute_in_term(self, term: str, x: str, U: str) -> str:
        term, x, U = norm(term), norm(x), norm(U)
        if not self.is_term(term):
            raise SyntaxErrorFOL(f"Not a term: {term!r}")
        ...
        return fill_template(template, [self.substitute_in_term(c, x, U) for c in children])
    ...
    def is_formula(self, expr: str) -> bool:
        ...
        # Nullary relation, e.g. A, B, or a subject-specific sentence-symbol.
        if template in self.relations and self.arity(template) == 0 and not children:
            return True

        # Atomic relation formula.
        if template in self.relations:
            return len(children) == self.arity(template) and all(self.is_term(c) for c in children)

        # Binary logical constructors.
        if template in (self.AND, self.OR, self.IMPLIES):
            return len(children) == 2 and all(self.is_formula(c) for c in children)

        # Quantifiers.
        if template in (self.FORALL, self.EXISTS):
            return len(children) == 2 and self.is_variable(children[0]) and self.is_formula(children[1])
    ...
    def substitute_in_formula(self, formula: str, x: str, U: str) -> str:
        ...
        formula, x, U = norm(formula), norm(x), norm(U)
        if not self.is_formula(formula):
            raise SyntaxErrorFOL(f"Not a formula: {formula!r}")
        if not self.is_variable(x):
            raise SyntaxErrorFOL(f"Not a variable: {x!r}")
        if not self.is_term(U):
            raise SyntaxErrorFOL(f"Not a term: {U!r}")
        return self._subst_formula_rec(formula, x, U, bound=set())

    def _subst_formula_rec(self, formula: str, x: str, U: str, bound: set[str]) -> str:
        if formula in (self.TRUE, self.FALSE):
            return formula
        template, children = decompose_top_level(formula)
        ...
        if template in (self.AND, self.OR, self.IMPLIES):
            return fill_template(
                template,
                [
                    self._subst_formula_rec(children[0], x, U, bound),
                    self._subst_formula_rec(children[1], x, U, bound),
                ],
            )
        ...
        if template in (self.FORALL, self.EXISTS):
            ...
            if binder in self.term_variables(U) and x in self.free_variables(scope):
                raise ProofError(
                    f"Unsafe substitution: variable {binder!r} in {U!r} would be captured in {formula!r}."
                )
            new_scope = self._subst_formula_rec(scope, x, U, bound | {binder})
            return fill_template(template, [binder, new_scope])

        raise SyntaxErrorFOL(f"Cannot substitute in formula: {formula!r}")

    def is_safe_substitution_result(self, source: str, target: str, x: str, U: str) -> bool:
        try:
    ...
    def equality_sides(self, eq_formula: str) -> Optional[tuple[str, str]]:
        ...
        try:
            ...
            return None
        if template == self.EQ and len(children) == 2 and all(self.is_term(c) for c in children):
            return children[0], children[1]
        return None

    def possible_equal_replacements(self, formula: str, old: str, new: str) -> set[str]:
        """All formulas obtained by safely replacing one term occurrence old by new."""
        formula, old, new = norm(formula), norm(old), norm(new)
        if not self.is_formula(formula):
            raise SyntaxErrorFOL(f"Not a formula: {formula!r}")
    ...
    def _replace_in_formula_rec(self, formula: str, old: str, new: str, bound: set[str]) -> set[str]:
        if formula in (self.TRUE, self.FALSE):
            return set()
        template, children = decompose_top_level(formula)
        results: set[str] = set()
        ...
        if template in (self.AND, self.OR, self.IMPLIES):
            for side in (0, 1):
                for new_side in self._replace_in_formula_rec(children[side], old, new, bound):
                    new_children = children[:]
        ...
        if template in (self.FORALL, self.EXISTS):
            binder, scope = children
            for new_scope in self._replace_in_formula_rec(scope, old, new, bound | {binder}):
                results.add(fill_template(template, [binder, new_scope]))
            return results

        return set()

    def _replace_in_term_rec(self, term: str, old: str, new: str, bound: set[str]) -> set[str]:
        results: set[str] = set()
        if term == old:
            if self.term_variables(new) & bound:
                # Some free variable of new would be captured by enclosing quantifier(s).
                return set()
            results.add(new)
    ...
    def possible_reformulations(self, formula: str, new_var: str) -> set[str]:
        """
        All formulas obtained by renaming one binding occurrence and the
        occurrences bound by it to a completely fresh variable new_var.
        """
        formula, new_var = norm(formula), norm(new_var)
        if not self.is_formula(formula):
            raise SyntaxErrorFOL(f"Not a formula: {formula!r}")
    ...
    def _all_variables_in_formula(self, formula: str) -> set[str]:
        ...
        if template in self.relations:
            ...
            return out
        if template in (self.AND, self.OR, self.IMPLIES):
            return self._all_variables_in_formula(children[0]) | self._all_variables_in_formula(children[1])
        if template in (self.FORALL, self.EXISTS):
    ...
    def _rename_bound_occurrences(self, formula: str, old: str, new: str) -> str:
        """Rename occurrences bound by the current binder old, stopping at shadowing binders."""
        if formula in (self.TRUE, self.FALSE):
            return formula
        template, children = decompose_top_level(formula)

        if template in self.relations:
            return fill_template(template, [self._rename_in_term(c, old, new) for c in children])
        ...
        if template in (self.FORALL, self.EXISTS):
            binder, scope = children
            if binder == old:
                # Shadowing: occurrences inside belong to the inner binder, not the outer one.
                return formula
            return fill_template(template, [binder, self._rename_bound_occurrences(scope, old, new)])
...
class Proof:
    ...
    Rule-specific examples:
        ...
        X.conclude(
            "[0]=[0]",
            rule="forall_elim",
            source="for every [x] we have [[x]=[x]]",
            var="x",
            term="0",
        )
        ...
        *,
        ...
        if self.goal_formula is not None:
            ...
            if self.parent is None:
                # A standalone proof with a goal: the proof body is numbered below
                # the goal line, e.g. 1.1, 1.2, ...
                self._goal_line = self._add_line("goal", self.goal_formula)
                self._prefix = self._goal_line.number
                self._next_number = 1
            else:
                # A subproof goal is a numbered line of the parent proof. The
                # subproof body uses subnumbers of that goal line.
    ...
    def _add_line(self, kind: LineKind, formula: str, rule: str = "", note: str = "") -> Line:
        line = Line(self._make_number(), kind, norm(formula), rule, note)
        self._next_number += 1
        self.lines.append(line)
        label = {"assumption": "Assume", "conclusion": "Conclude", "goal": "Goal"}[kind]
    ...
    def available_formulas(self) -> list[str]:
        ...
        for line in self.lines:
            if line.kind in ("assumption", "conclusion") or (line.kind == "goal" and line.proved):
    ...
    def local_conclusions(self) -> list[str]:
        return [line.formula for line in self.lines if line.kind == "conclusion"]
    ...
    def _check_conclusion(self, formula: str, *, rule: Optional[str], **kwargs: str) -> str:
        ...
        # Try automatic rules in a pedagogically useful order.
        for checker in (
    ...
    def _check_named_rule(self, formula: str, rule: str, **kwargs: str) -> str:
        ...
        if rule in {"forall_elim", "universal_elim"}:
            ...
            if not self.language.term_variables(term) <= self.contextual_variables():
                raise ProofError("Every variable of the substituting term must be contextual.")
            if not self.language.is_safe_substitution_result(source_scope_of_forall(source, self.language, var), formula, var, term):
        ...
        if rule in {"exists_intro", "existential_intro"}:
            ...
            if var != bound_var:
                raise ProofError("var=... must be the variable bound by the existential conclusion.")
            ...
            if not self.language.term_variables(term) <= self.contextual_variables():
                raise ProofError("Every variable of the witnessing term must be contextual.")
        ...
        if rule in {"exists_elim", "existential_elim", "witness"}:
            ...
            if witness in self.contextual_variables():
                raise ProofError("The witness variable must not be contextual.")
            template, children = decompose_top_level(source)
            if template != self.language.EXISTS or len(children) != 2:
                raise ProofError("Source is not existential.")
            bound_var, scope = children
            if var != bound_var:
                raise ProofError("var=... must be the variable bound by the existential source.")
        ...
        if rule in {"equality_elim", "eq_elim", "substitution_of_equals"}:
            ...
            if not self._in_context(equality) or not self._in_context(source):
                raise ProofError("Equality and source formula must both be in the context.")
            ...
            if formula not in possible:
                raise ProofError("Formula is not obtained by a safe replacement of equals.")
        ...
        if rule in {"reformulation", "alpha", "alpha_conversion"}:
            ...
            if new_var in self.contextual_variables():
                raise ProofError("The new variable must not be contextual.")
    ...
    def _auto_reiteration(self, formula: str) -> Optional[str]:
        return "available formula" if self._in_context(formula) else None
    ...
    def _auto_false_elim(self, formula: str) -> Optional[str]:
        return "false elimination" if self.language.FALSE in self.available_formulas() else None
    ...
    def _auto_forall_elim(self, formula: str) -> Optional[str]:
        ...
        for ctx in self.available_formulas():
            ...
            for U in witnesses:
                if U is None:
                    return "universal elimination"
                if self.language.is_term(U) and self.language.term_variables(U) <= self.contextual_variables():
                    try:
    ...
    def _auto_exists_elim(self, formula: str) -> Optional[str]:
        """Automatically detect the current-system witness form of existential elimination."""
        for ctx in self.available_formulas():
            try:
                template, children = decompose_top_level(ctx)
            except SyntaxErrorFOL:
            ...
            for y in witnesses:
                if y is None:
                    continue
                if self.language.is_variable(y) and y not in self.contextual_variables():
                    try:
    ...
    def _auto_reformulation(self, formula: str) -> Optional[str]:
        ...
        candidate_new_vars = self.language.variables
        for source in self.available_formulas():
            for new_var in candidate_new_vars:
                if new_var in self.contextual_variables():
                    continue
                try:
    ...
    def _infer_substitution_terms(self, pattern: str, instance: str, x: str) -> set[Optional[str]]:
        """
        Infer terms U such that pattern[U/x] could equal instance.

        Returns {None} only in the special case where x is not free in the
        pattern and pattern is already identical to the instance.
        """
        pattern, instance, x = norm(pattern), norm(instance), norm(x)
    ...
    def _match_formula_instance(self, pattern: str, instance: str, x: str, bindings: set[str]) -> bool:
    ...
    def _match_term_instance(self, pattern: str, instance: str, x: str, bindings: set[str]) -> bool:
    ...
    def _auto_double_negation(self, formula: str) -> Optional[str]:
        target = fill_template(
            ...
            [fill_template(self.language.IMPLIES, [formula, self.language.FALSE]), self.language.FALSE],
        )
    ...
    def _check_goal_subproof(self, subproof: Proof) -> None:
        ...
        if goal not in subproof.local_conclusions():
            # For implication and universal introduction, the goal may be justified
            # by the shape of the subproof even if not directly concluded inside it.
            pass
        ...
        if template == self.language.IMPLIES and len(children) == 2:
            ...
            if assumptions != [A]:
                raise ProofError("Implication-introduction subproof must have A as its only assumption line.")
            if B not in subproof.local_conclusions():
                raise ProofError("Implication-introduction subproof must conclude B.")
            # Free variables of implication must be contextual in the parent.
            if not self.language.free_variables(goal) <= self.contextual_variables():
                raise ProofError("Free variables of the implication goal must be contextual.")
            return

        # Universal introduction.
        if template == self.language.FORALL and len(children) == 2:
            x, A = children
            parent_contextual = self.contextual_variables()
            free_A = self.language.free_variables(A)
            if not (free_A - {x}) <= parent_contextual:
                raise ProofError("All free variables of A except x must be contextual.")
            if x in parent_contextual:
                raise ProofError("The universal variable x must not be contextual.")
            if subproof.local_assumptions():
                raise ProofError("Universal-introduction subproof must not introduce assumptions.")
            if A not in subproof.local_conclusions():
                raise ProofError("Universal-introduction subproof must conclude A.")
            return

        # General goal: the goal itself must have been concluded in the subproof.
        if goal not in subproof.local_conclusions():
